...
- CLI persistence fix: `main()`/`_cli()` were building a fresh in-memory `Kernel()` per invocation, so `Executive._on_kernel_started`'s cold-start guard had no history to see (not a guard bug — nothing persisted). `_cli()` now wires `Kernel(event_log=EventLog(path=<--data-dir>/event_log.jsonl))`, default `--data-dir aether_data`, threaded into both subcommands; `main()` takes an optional `kernel` param (default `None` preserves existing fresh-in-memory library/test semantics). Verified via two sequential `python main.py explore` subprocess runs against the same `--data-dir`: second run's replay reproduces the first run's history byte-for-byte (confirmed no new cold-start-rationale lines land in the persisted log), then proceeds live from where the first run left off. Commit: `<pending>`.
